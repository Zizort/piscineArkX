...
async function updateByName(Name, Price) {
    try {
...
async function deleteDate() {
    try {
        const result = await Product.deleteMany({ expirationDate: { $lt: Date.now() } });
...
async function updateDescription() {
    try {
        ...
        for (let i = 0; i < result.length; i++) {
            result[i].description = result[i].description + ' (updated)';
            result[i].save();
        }
